...
export default function Dashboard(props: DashboardProps) {
    ...
    const trafficData = getLast7Days().map((day) => {
        const matchingDate = Object.entries(traffic).find(([date]) => date.split('T')[0] === day.fullDate);
    ...
    const jobAppliedTraffic = getLast7Days().map((day) => {
        const matchingDate = Object.entries(job_applied).find(([date]) => date.split('T')[0] === day.fullDate);
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <p className="text-muted-foreground text-xs">Users with "candidate" role</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">{Object.values(job_applied).reduce((sum, count) => sum + count, 0)}</div>
                            <p className="text-muted-foreground text-xs">Total jobs applied over time</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <CardContent>
                            ...
                            <p className="text-muted-foreground text-xs">User test taken</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <CardContent>
                            ...
                            <p className="text-muted-foreground text-xs">User interview scheduled</p>
                        </CardContent>
                    </Card>
                </div>

                {/* Traffic Charts - Side by side */}
                <div className="grid gap-4 pt-2 md:grid-cols-2 lg:grid-cols-2">
                    {/* First Chart - User Registration */}
                    <Card className="grid gap-4">
                        <CardHeader>
                            <CardTitle>User Registration Traffic</CardTitle>
                            <CardDescription>User registration activity over the last 7 days</CardDescription>
                        </CardHeader>
                        <CardContent>
                            {/* Recharts Area Chart */}
                                ...
                                <div className="h-64">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={trafficData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                            ...
                                            <YAxis />
                                            <Tooltip />
                                            <Area type="monotone" dataKey="users" stroke="#82ca9d" fill="#82ca9d50" />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            ) : (
                    ...
                    <Card className="grid gap-4">
                        <CardHeader>
                            ...
                            <CardDescription>Job application activity over the last 7 days</CardDescription>
                        </CardHeader>
                        <CardContent>
                            {jobAppliedTraffic.length > 0 ? (
                                <div className="h-64">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={jobAppliedTraffic} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                            <CartesianGrid strokeDasharray="3 3" />
                                            <XAxis dataKey="date" />
                                            <YAxis />
                                            <Tooltip />
                                            <Area type="monotone" dataKey="users" name="Jobs" stroke="#0E9FD8FF" fill="#033BD650" />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            ) : (
